...
The private schedule remains every 15 minutes (`2,17,32,47`). The run ceiling is 70 Browse searches, not a fixed spend: the live quota is paced over runs remaining until eBay's reset, subtracting an 80-call safety reserve and 28 projected shared calls/hour (26 charity seller searches plus two market searches). This targets roughly 4,100–4,300 private calls/day under normal one-page charity scans. Quota lookup failure, or a missing/stale reset, limits a run to 20 calls (and never exceeds a known remaining allowance minus the reserve).

A typical 44-call plan protects 16 long-tail library queries, 10 active-stock pages, two contemporary and two classic contributor-only queries, and every other search family. Spare quota expands to a 70-query plan: 16 library titles, 12 active-stock pages, six contemporary and six classic hot titles, four contributor searches, four auctions, four broad queries, four collectible-format queries, four collection queries and ten wrong-category queries. Lane cursors advance only over selected prefixes, so quota trimming cannot silently skip library records or inventory pages. Priority contributor searches cover the 14 configured core photographers in seven normal runs; the contemporary lane separately rotates the library's highest-priority contemporary contributors.

Active-stock coverage now rotates ten query phrases at 200 results/page through offset 9,800 (eBay's existing 10,000-result window), about 12.5 hours per full sweep at ten pages/run, or 10.5 hours at twelve. The long-tail pool currently contains 4,206 of the 4,318 titles (112 hot titles have their own rotation): sixteen/run gives a theoretical 2.74-day cycle, versus 5.48 days at eight/run. These are healthy-quota estimates, not delivery guarantees. Charity pagination, API retries, manual runs, restarted backfills and scheduler delays can reduce throughput; shared usage is re-read each private run. Unknown quota uses a conservative cap, not a guaranteed global reservation. No ChatGPT schedules are changed.
...
`ebay_seller_monitor.py` separately checks 103 selected charity and library sellers: 89 on eBay UK and 14 on eBay US. The hourly workflow processes 26 sellers in a persistent round-robin batch (previously 51), so every seller is normally revisited within four runs. At one page/seller this uses 624 calls/day instead of 1,224. Each seller gets an independent Books-category query, so a large seller cannot consume a shared 200-result page and hide stock from smaller shops. Incremental scans can use up to five pages/seller; the existing 450-call charity guard and worst-case page headroom checks remain in place. The US searches also require delivery availability to Great Britain.
